...
from typing import Any, Callable, Coroutine, List, Optional, Set, TYPE_CHECKING
...
class AsyncExecutor:
    ...
    def _spin_once(self) -> None:
        ...
        for node in nodes_to_use:
            ...
            timers.extend(
                filter(lambda e: self._can_execute(e), node.timers)
            )
            clients.extend(
                filter(lambda e: self._can_execute(e), node.clients)
            )
            services.extend(
                filter(lambda e: self._can_execute(e), node.services)
            )
            node_guards = filter(lambda e: self._can_execute(e), node.guards)
            waitables.extend(
                filter(lambda e: self._can_execute(e), node.waitables)
            )
    ...
    def _execute_timer(self, tmr: Timer) -> None:
        ...
        try:
            with tmr.handle:
                ...
                self._task_group.start_soon(_execute)
        except (InvalidHandle, TimerCancelledError):
            pass
    
    def _execute_subscription(self, sub: Subscription[Any]) -> None:
    ...
    def _execute_client(self, client: Client[Any, Any]) -> None:
    ...
    def _execute_service(self, srv: Service[Any, Any]) -> None:
    ...
    def _execute_waitable(self, waitable: Waitable[Any]) -> None:
    ...
    async def __aenter__(self) -> 'AsyncExecutor':
        """Enter the async context and start the spin loop."""
    ...
    async def __aexit__(
        ...
        """Exit the async context and clean up."""
